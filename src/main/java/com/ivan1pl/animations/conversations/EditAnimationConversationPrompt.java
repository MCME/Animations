...
public class EditAnimationConversationPrompt extends BaseEditorValidatingPrompt {
    ...
    protected Prompt acceptValidatedInput(ConversationContext cc, String string) {
        ...
        StationaryAnimation sAnimation =
                animation instanceof StationaryAnimation ? (StationaryAnimation) animation : null;
        simplePrompt = true;
            ...
            for (ConversationCommandHandler handler : STATIONARY_EDIT_COMMANDS) {
                if (realInput[0].equalsIgnoreCase(handler.getName())) {
                    try {
                        ...
                        Logger.getLogger(EditAnimationConversationPrompt.class.getName())
    ...
    public String getPromptText(ConversationContext cc) {
        ...
        if (isEdit && !simplePrompt) {
            if (sAnimation != null) {
                message = MessageUtil.formatPromptMessage(
                        Messages.MSG_STATIONARY_ANIMATION_EDIT_INFO,
                        name,
                        animation.getFrameCount(),
                        animation.getInterval(),
                        formatAnswers(STATIONARY_EDIT_COMMANDS));
            } else if (mAnimation != null) {
                message = MessageUtil.formatPromptMessage(
                        Messages.MSG_MOVING_ANIMATION_EDIT_INFO,
                        name,
                        mAnimation.getStepX(),
                        mAnimation.getStepY(),
                        mAnimation.getStepZ(),
                        mAnimation.getMaxDistance(),
                        animation.getInterval(),
                        formatAnswers(MOVING_EDIT_COMMANDS));
            } else {
            ...
            if (sAnimation != null) {
                message = MessageUtil.formatPromptMessage(
                        Messages.MSG_STATIONARY_ANIMATION_EDIT_INFO_SIMPLE,
                        name,
                        animation.getFrameCount(),
                        animation.getInterval());
            } else if (mAnimation != null) {
                message = MessageUtil.formatPromptMessage(
                        Messages.MSG_MOVING_ANIMATION_EDIT_INFO_SIMPLE,
                        name,
                        mAnimation.getStepX(),
                        mAnimation.getStepY(),
                        mAnimation.getStepZ(),
            ...
            return MessageUtil.formatPromptMessage(
                    Messages.MSG_CREATE_ANIMATION,
                    name,
                    Animations.getWandMaterial().toString());
        }
        message += "\n";
        if (animation.getTriggerBuilderData() != null) {
            if (animation.getTriggerBuilderData().getType() == TriggerType.TWO_BLOCK) {
                message += MessageUtil.formatPromptMessage(
                        Messages.TRIGGER_NORANGE_INFO,
                        animation.getTriggerBuilderData().getType().toString().toLowerCase());
            } else {
                message += MessageUtil.formatPromptMessage(
                        Messages.TRIGGER_INFO,
                        animation.getTriggerBuilderData().getType().toString().toLowerCase(),
                        animation.getTriggerBuilderData().getRange());
            }
        } else {
        ...
        if (animation.getSoundData() != null) {
            message += MessageUtil.formatPromptMessage(
                    Messages.SOUND_INFO,
                    animation.getSoundData().getName(),
                    animation.getSoundData().getPlayMode().toString(),
                    animation.getSoundData().getRange(),
                    animation.getSoundData().getPitch(),
                    animation.getSoundData().getVolume());
        } else {
    ...
    protected boolean isInputValid(ConversationContext cc, String string) {
        ...
        for (ConversationCommandHandler handler : cmpList) {
            if (handler.getName().equalsIgnoreCase(realInput[0])
                    && handler.getParamsCount() <= realInput.length - 1
