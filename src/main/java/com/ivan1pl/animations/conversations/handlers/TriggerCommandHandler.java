...
public class TriggerCommandHandler extends ConversationCommandHandler {
    ...
    public Prompt handle(ConversationContext cc, Animation animation, String animationName, String[] params)
        ...
        Prompt retPrompt = new ConversationResponsePrompt(
                successPrompt, MessageUtil.formatInfoMessage(Messages.MSG_TRIGGER_CHANGED));
        TriggerBuilder builder =
                new TriggerBuilder(animation).setTriggerType(type).setRange(i);
        if (i < 0
                && (type == TriggerType.BLOCK
                        || type == TriggerType.LOOP
                        || type == TriggerType.PASSWORD
                        || type == TriggerType.RANGE
                        || type == TriggerType.CHAIN)) {
            return new ConversationResponsePrompt(
                    failPrompt, MessageUtil.formatErrorMessage(Messages.MSG_RANGE_NEEDED, type.toString()));
        }
        if (null != type)
            switch (type) {
                case BLOCK:
