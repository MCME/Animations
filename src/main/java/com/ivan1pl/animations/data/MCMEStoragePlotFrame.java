...
public class MCMEStoragePlotFrame implements IFrame, IStoragePlot {
    ...
    public void show(int offsetX, int offsetY, int offsetZ) {
        try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(frameNBTData))) {
            new MCMEPlotFormat()
                    .load(
                            this.getLowCorner().add(new Vector(offsetX, offsetY, offsetZ)),
                            0,
                            new boolean[3],
                            true,
                            false,
    ...
    public void setBlocks(BlockData[] blockData) {
        ...
        try (ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
                DataOutputStream out = new DataOutputStream(byteOut)) {
            new MCMEPlotFormat().save(this, out, new StoragePlotSnapshotFaksimile(this, blockData));
            out.flush();
            out.close();
            byteOut.flush();
            byteOut.close();
    ...
    public static class StoragePlotSnapshotFaksimile extends StoragePlotSnapshot {
        ...
        public StoragePlotSnapshotFaksimile(IStoragePlot plot, BlockData[] blockData) {
            super(plot);
            this.plot = plot;
