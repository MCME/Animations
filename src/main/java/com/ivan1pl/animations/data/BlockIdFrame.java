...
public class BlockIdFrame implements Serializable, IFrame {
    ...
    private int y;
    private int z;
    private transient UUID worldId;
    ...
    public void show(int offsetX, int offsetY, int offsetZ) {
        throw new UnsupportedOperationException(
                "Showing BlockIdFrames is no longer supported in 1.13."); // To change body of generated methods, choose
        // Tools | Templates.
    }

    @Override
    public boolean isInside(Location location, int offsetX, int offsetY, int offsetZ) {
        return (location.getBlockX() >= x + offsetX
                && location.getBlockX() < x + sizeX + offsetX
                && location.getBlockY() >= y + offsetY
                && location.getBlockY() <= y + sizeY + offsetY
                && location.getBlockZ() >= z + offsetZ
                && location.getBlockZ() < z + sizeZ + offsetZ);
    }

    public static BlockIdFrame fromSelection(Selection s) {
        if (Selection.isValid(s)) {
            throw new UnsupportedOperationException(
                    "Creating BlockIdFrames from Selections is no longer supported in 1.13."); // To change body of
            // generated methods,
            // choose Tools |
            // Templates.
        }
    ...
    public void setBlocks(Material[] blockMaterials, Byte[] blockData) {
        throw new UnsupportedOperationException(
                "Creating BlockIdFrames from is no longer supported in 1.13."); // To change body of generated methods,
    ...
    private BlockData getNeighbour(int i, int j, int k) {
        if (i >= 0 && i < sizeX && j >= 0 && j < sizeY && k >= 0 && k < sizeZ) {
            ...
            BlockData result =
                    Bukkit.getWorld(worldName).getBlockAt(x + i, y + j, z + k).getBlockData();
            return result;
        }
    }
